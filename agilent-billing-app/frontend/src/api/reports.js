...
export async function fetchRevenueMix(month) {
  const { data } = await client.get("/reports/revenue-mix", { params: { month } });
  return data;
}

export async function fetchVehicleWiseBreakup(month) {
  const { data } = await client.get("/reports/vehicle-wise-breakup", { params: { month } });
  return data;
}

export async function fetchOwnershipBreakup(month) {
  const { data } = await client.get("/reports/ownership-breakup", { params: { month } });
  return data;
}
...
export function downloadReportUrl(reportType, month) {
  return `/api/export/${reportType}?month=${encodeURIComponent(month)}`;
}
